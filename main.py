...
if __name__ == '__main__':
    ...
    # Logging important configuration details
    log_config = f"""
    ======== Experiment Configuration ========
    Dataset: {args.dataset}
    Alpha (Data Percent): {args.datapercent}
    Batch Size: {args.batch}
    Learning Rate: {args.lr}
    Number of Clients: {args.n_clients}
    Iterations (Communication Rounds): {args.iters}
    Local Worker Epochs: {args.wk_iters}
    Test Environments (Clients): {args.test_envs}
    Model: {args.net}
    ...
    training_images = sum(len(loader.dataset) for loader in train_loaders)
    validation_images = sum(len(loader.dataset) for loader in val_loaders)
    testing_images = sum(len(loader.dataset) for loader in test_loaders)

    image_log = f"""
    ======== Dataset Statistics ========
    Total Training Images: {training_images}
    Total Validation Images: {validation_images}
            ...
            for client_idx, model in enumerate(models):
                if client_idx in args.test_envs:
                    ...
                    print(' Site-{:d}| Val  Acc: {:.4f}'.format(
                        client_idx, val_acc[0]), flush=True)
                    logrecord += ' Site-{:d}| Val  Acc: {:.4f}\n'.format(
                        client_idx, val_acc[0])
            # best_acc = 0
